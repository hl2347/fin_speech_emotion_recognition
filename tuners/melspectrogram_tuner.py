...
tuner = kt.RandomSearch(
    ...
    overwrite=True,
    directory="tuners",
    project_name="melspectrogram_tuner",
)

#label input data
mel_list = []
for i in input_data:
    mel_list.append(librosa.feature.melspectrogram(y=i,n_fft=2048, hop_length=4512,center=False).T)

mel_list = np.array(mel_list)
mel_list = mel_list.reshape(204400,128)
...
# split data
mel_x_train, mel_x_rem, mel_y_train, mel_y_rem = train_test_split(mel_X, mel_y, random_state=0, shuffle=True, train_size=0.7)
mel_x_val, mel_x_test, mel_y_val,mela_y_test = train_test_split(mel_x_rem, mel_y_rem, random_state=0, shuffle=True, train_size=0.5)

tuner.search(mel_x_train, mel_y_train, epochs=50, validation_data=(mel_x_val, mel_y_val))
...
best_model.save("melspectrogram_tuned1")
second_model.save("melspectrogram_tuned2")