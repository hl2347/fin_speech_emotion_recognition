...
mel_list = []
for i in input_data:
    mel_list.append(librosa.feature.melspectrogram(y=i,n_fft=4096, hop_length=4096).T)

mel_list = np.array(mel_list)
mel_list = mel_list.reshape(len(input_data),128)
print("reshaped mel list")
...
#mel model
model = Sequential()
model.add(Dense(1024,input_shape=(2048,), activation='relu', name="mel_1"))
model.add(Dense(320, activation='relu',name="mel_2"))
model.add(Dense(672, activation='relu',name="mel_3"))
model.add(Dense(704, activation='relu',name="mel_4"))
model.add(Dense(640, activation='relu',name="mel_5"))
model.add(Dense(832, activation='relu',name="mel_6"))
model.add(Dense(128, activation='relu',name="output"))
...
history = model.fit(mel_x_train, mel_y_train, batch_size=64, epochs=50, validation_data=(mel_x_val, mel_y_val), callbacks=[rlrp])

pred = model.predict(mel_x_test)
print("r squared value: " + str(r2_score(mel_y_test, pred)))

print("MSE of our model on test data : " , model.evaluate(mel_x_test,mel_y_test)[1]*100 , "%")
...
plt.show()
plt.savefig("FIN_melspectrogram_training_full_dataset.png")
model.save("FIN_melspectrogram_training_full_dataset")