...
model.add(MaxPooling1D(pool_size=256, strides = 110, padding = 'valid'))
model.add(Flatten())
...
history = model.fit(mel_x_train, mel_y_train, batch_size=256, epochs=50, validation_data=(mel_x_val, mel_y_val), callbacks=[rlrp])
...
plt.show()
plt.savefig("FIN_mel_training_test_7.png")
model.save("FIN_mel_training_test_7")