...
file_title = sys.argv[0].split('.')[0]
print(file_title)

comb_df = pd.read_csv("mel_kernel_combinations.csv")
kernel_size = 1024
pool_strides = 128
pool_size= 256
print(kernel_size)
...
def oob(test, pred, sensitivity=None):
    ...
    melspectrogram_std = np.sqrt(melspectrogram_var)
    sensitivity = melspectrogram_std/5
...
label_base_directory = "../../frame_features/melspectrogram/"
...
train_label = np.array(train_label)[:,64:]
melspectrogram_var = np.var(train_label,axis=0)
...
val_label = np.array(val_label)[:,64:]
...
test_label = np.array(test_label)[:,64:]
...
model.add(Dense(units=64, activation='relu'))
model.compile(optimizer=optimizers.Adam(learning_rate=1e-4), loss="mae", metrics=["mse","mae"])
...
print("r squared value variance weighted: " + str(r_squared_var_weighted))
print("oob std weighted: "+  str(oob_value))
# print("oob 0.005: "+  str(oob(test_label,pred,0.005)))
...
plt.show()
plt.savefig("figures_2/"+file_title+".png")
model.save("models_2/"+file_title)
...
#     results.to_csv("results_2.csv",mode='a',index=False,header=None)
# except:
