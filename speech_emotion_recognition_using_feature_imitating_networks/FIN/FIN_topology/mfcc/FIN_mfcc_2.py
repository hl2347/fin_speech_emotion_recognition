...
#rms model
model=Sequential()
model.add(Conv1D(256, kernel_size=128, strides=1, padding='valid', activation='relu', input_shape=(2048, 1), name="mfcc_1"))
model.add(MaxPooling1D(pool_size=256, strides = 32, padding = 'valid', name="mfcc_max"))
model.add(Flatten())
model.add(Dense(units=64, activation='relu', name="mfcc_2"))
model.add(Dense(units=64, activation='relu', name="mfcc_3"))
model.add(Dense(units=20, activation='linear',name="mfcc_output"))
...
print("oob std weighted: "+  str(oob(test_label,pred)))
# print("oob 5: "+  str(oob(test_label,pred,5)))
# print("oob 1: "+  str(oob(test_label,pred,1)))
...
plt.show()
plt.savefig("FIN_mfcc_2.png")
model.save("FIN_mfcc_2")