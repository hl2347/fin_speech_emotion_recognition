...
comb_df = pd.read_csv("zcr_kernel_combinations.csv")
kernel_size = int(comb_df.iloc[file_index,0])
strides = int(comb_df.iloc[file_index,1])
print(kernel_size)
print(strides)
...
#zcr model
model = Sequential()
model.add(Conv1D(16,kernel_size=kernel_size,strides=strides, padding='valid', activation='relu',name='zcr_1',input_shape=(2048, 1)))
model.add(Flatten())
...
try:
    ...
    # r_squared
    # oob_value
    results = pd.DataFrame([[file_index,mse,val_mse,mae,val_mae,test_mse,r_squared,oob_value]],columns=["file_index","mse","val_mse","mae","val_mae","test_mse","r_squared","oob_value"])
